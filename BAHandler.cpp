...
int min_imgpt_else_pruned = 2;   // points must be seen in at least this number of views
...
namespace
{
	...
	{
		int const kk = measurements.size();
		...
		cout << "mean reprojection error (in pixels): " << meanReprojectionError/kk << endl;
	}
    
    
}

int BAHandler::Count2DMeasurements(const vector<CloudPoint>& pointcloud) {
	int k = 0;
...
// ntk
int BAHandler::Count2DMeasurements_pruned(const vector<CloudPoint>& pointcloud) {
	int k = 0;
	for (unsigned int i=0; i<pointcloud.size(); i++) {
        int nimgpt = 0;
		for (unsigned int ii=0; ii<pointcloud[i].imgpt_for_img.size(); ii++) {
        ...
        if (nimgpt >= min_imgpt_else_pruned) {
            k++;
        } 
	}
	return k;
...
void BAHandler::adjustBundle(vector<CloudPoint>& pointcloud,
								  Mat& cam_matrix,
								...
								) 
{
	...
	if(good_adjustment) {
		...
		for (int i = 0; i < N; ++i)
		{
            ...
            cout << "((((((((((((Pmats[i] = " << Pmats[i] << ",  i = " << i <<  ",  N = " << N << endl;
...
void BAHandler::adjustBundle_vid(vector<CloudPoint>& pointcloud,
                             Mat& cam_matrix,
                             const std::vector<std::vector<cv::KeyPoint> >& imgpts,
                             std::map<int ,cv::Matx34d>& Pmats,
                             std::vector<std::vector <int> >& mapViews,
                             int nCams
                             )
{
    // ntk
    ...
    int N = nCams, M = pointcloud.size(), K = Count2DMeasurements(pointcloud);
    ...
    **/
	...
	{
		////// int camId = i;     // ntk
        int camId = mapViews[i][0];
        
		Matrix3x3d R;
		Vector3d T;
		
		////// Matx34d& P = Pmats[i];
        Matx34d& P = Pmats[camId];
	...
	if(good_adjustment) {
		...
		for (int i = 0; i < N; ++i)
		{
			...
			///// Pmats[i] = P;     // ntk
            Pmats[mapViews[i][0]] = P;
            
            cout << "((((((((((((Pmats[i] = " << Pmats[mapViews[i][0]] << ",  i = " << i <<  ",  N = " << N << endl;
...
void BAHandler::adjustBundle_pruned(vector<CloudPoint>& pointcloud,
                            Mat& cam_matrix,
                            const std::vector<std::vector<cv::KeyPoint> >& imgpts,
                            std::map<int ,cv::Matx34d>& Pmats
                            )
{
	int N = Pmats.size(), M = pointcloud.size(), K = Count2DMeasurements_pruned(pointcloud);
	...
	for (int j = 0; j < M; ++j)
	{
        ...
        // ntk
        int nimgpt = 0;
        for (unsigned int ii=0; ii<pointcloud[j].imgpt_for_img.size(); ii++) {
                if (pointcloud[j].imgpt_for_img[ii] >= 0) {
                    nimgpt++;
                }
        }
        if (nimgpt >= min_imgpt_else_pruned) {
            Xs[pointId][0] = pointcloud[j].pt.x;
            Xs[pointId][1] = pointcloud[j].pt.y;
            Xs[pointId][2] = pointcloud[j].pt.z;
            pointIdFwdMap[j] = pointId;
            pointIdBwdMap.insert(make_pair(pointId, j));
            pointId++;
        } else {
	...
	for (unsigned int k = 0; k < pointcloud.size(); ++k)
	{
        ...
        // ntk
        int nimgpt = 0;
        for (unsigned int ii=0; ii<pointcloud[k].imgpt_for_img.size(); ii++) {
            if (pointcloud[k].imgpt_for_img[ii] >= 0) {
                nimgpt++;
            }
        }
	...
	if(good_adjustment) {
        ...
        std::cout << "inside good_adjustment of adjustBundle_prune" << "\n";
        
        // ntk
        int count = 0;
        for (unsigned int jj=0; jj<pointcloud.size(); jj++) {
        
           int nimgpt = 0;
           for (unsigned int ii=0; ii<pointcloud[jj].imgpt_for_img.size(); ii++) {
            if (pointcloud[jj].imgpt_for_img[ii] >= 0) {
                nimgpt++;
            }
           }
           if (nimgpt >= min_imgpt_else_pruned) {
		      ...
		      //extract 3D points
		      /////for (unsigned int j = 0; j < Xs.size(); ++j) {
			     pointcloud[jj].pt.x = Xs[count][0];
			     pointcloud[jj].pt.y = Xs[count][1];
			     pointcloud[jj].pt.z = Xs[count][2];
                 count++;
		      /////}
           } else {
               
               pointcloud[jj].pt.x = 0.0;
               pointcloud[jj].pt.y = 0.0;
               pointcloud[jj].pt.z = 0.0;
               
               std::cout << "pointcloud set to (0.0, 0.0, 0.0) ....................... \n";
